...
//  srv_like is T2.4A from Francis (2011)
...
FUNCTION dvar_vector SRecruit(const dvar_vector& Stmp)
  ...
  dvar_vector RecTmp(Stmp.indexmin(),Stmp.indexmax());
    ...
    case 2:
      ...
      break;
    case 3:
      RecTmp = mfexp(mean_log_rec(iarea));                             //Avg recruitment (Rzero(i_area)* removed)
      break;
    case 4:
...
FUNCTION Srv_Like
  ...
  for (int iarea=1;iarea<=nareas;iarea++)
    for (int iyr=1;iyr<=nyrs_srv(isrv,iarea);iyr++)
      ...
      surv_like(isrv,iarea) += log(sqrt(log(1.0+obs_se_srv(isrv,iarea,iyr)/obs_srv(isrv,iarea,iyr))))+
                                0.5*square(log(obs_srv(isrv,iarea,iyr)/pred_srv(isrv,iarea,yrs_srv(isrv,iarea,iyr))) / 
                                sqrt(log(1.0+obs_se_srv(isrv,iarea,iyr)/obs_srv(isrv,iarea,iyr)))+
				(0.5*sqrt(log(1.0+obs_se_srv(isrv,iarea,iyr)/obs_srv(isrv,iarea,iyr)))));
  if (DebugOut > 0 ) {
